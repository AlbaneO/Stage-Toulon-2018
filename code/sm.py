...
class RobotBehavior(object):
	...
	def __init__(self, handle_pub, T):
		...
		self.fs = fsm([ ("Start","JoyControl", True ),
			...
			("ChangementDirection","Retreat",self.check_ChangementDirection_To_Retreat,self.DoRetreat),
			("ChangementDirection","JoyControl",self.check_To_JoyControl,self.DoJoyControl),
			("ChangementDirection","OnGround",self.check_To_OnGround,self.DoOnGround),
	...
	def check_ChangementDirection_To_Retreat(self,fss):
		return (self.bumper_activated)
	...
	def KeepAutonomousMode(self,fss):
		...
		if (self.message and self.message_info):
			sys.stdout.write("\033[36m")
			print "KeepAutonomousMode",self.check_To_JoyControl(fss),self.check_To_OnGround(fss),self.check_AutonomousMode_To_Retreat(fss),self.check_AutonomousMode_To_ChangementDirection(fss)
			sys.stdout.write("\033[37m")
	...
	def KeepChangementDirection(self,fss):
		...
		self.next_state.append(self.check_ChangementDirection_To_Retreat(fss))
		if (self.message and self.message_info):
			sys.stdout.write("\033[36m")
			print "KeepChangementDirection",self.check_To_JoyControl(fss),self.check_To_OnGround(fss),self.check_ChangementDirection_To_AutonomousMode(fss),self.check_ChangementDirection_To_Retreat(fss)
			sys.stdout.write("\033[37m")
		if self.check_To_JoyControl(fss):
			return False
		else:
			if self.check_To_OnGround(fss):
				...
				if self.check_ChangementDirection_To_AutonomousMode(fss):
					return False
				else:
					return (not (self.check_ChangementDirection_To_Retreat(fss)))
